...
STEP_B = "step_b_criterion_mapping"                     # variante estricta (firmada)
STEP_B_NONSTRICT = "step_b_criterion_mapping_nonstrict"  # experimento diagnóstico B4b §6(B)
CRITIC = "critic"


def step_b_id(variant: str = "strict") -> str:
    return STEP_B if variant == "strict" else STEP_B_NONSTRICT
...
def assert_all_signed(*, variant: str = "strict") -> None:
    step_b = step_b_id(variant)
    unsigned = [p for p in (STEP_A, step_b, CRITIC) if not is_signed(p)]
    if unsigned:
        raise PromptNotSignedError(
            f"prompts de juicio V2 sin firmar (variante {variant}): {unsigned}. "
            f"B4b no puede correr (ver docs_plan/PROPUESTA_PROMPTS_JUICIO_V2_B4.md).")
