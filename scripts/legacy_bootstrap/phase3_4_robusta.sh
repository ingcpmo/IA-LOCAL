...
REPORT_FILE="${LOG_DIR}/phase3-4-robusta-$(date +%Y%m%d-%H%M%S).log"
...
main() {
  log "ARIA Phase 3+4 Robusta (Ollama nativo + builds condicionales)"
  ...
  if [[ ${#EXISTING_SERVICES[@]} -eq 0 ]]; then
    ...
    for svc in "${EXISTING_SERVICES[@]}"; do
      docker image inspect "${PROJECT_NAME}-${svc}" >/dev/null 2>&1 || fail "Falta imagen ${PROJECT_NAME}-${svc}"
      pass "Imagen ${PROJECT_NAME}-${svc} OK"
    done
  fi
